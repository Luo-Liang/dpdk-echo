...
int port_init(lcore_args *larg, std::string srcIp, std::string srcMac, std::vector<std::string> blockedSrcMac)
{
    ...
    for (int i = 0; i < nb_ports; i++)
    {
        ...
        {
            portids.push_back(i);
            larg->associatedPorts.push_back(i);
            break;
        }
    }
    ...
    larg->pool = rte_pktmbuf_pool_create(bufpool_name,
                                         ...
                                         RTE_MBUF_DEFAULT_BUF_SIZE, rte_lcore_to_socket_id(larg->CoreID));
    if (larg->pool == NULL)
    {
        rte_exit(EXIT_FAILURE, "Error: rte_pktmbuf_pool_create failed\n");
    }
    //largs[i].src_id = (int *)malloc(sizeof(int) * nb_ports);
    larg->srcs.resize(larg->associatedPorts.size());
    //largs[i].srcMacs.resize(nb_ports);
    for (size_t pidx = 0; pidx < larg->associatedPorts.size(); pidx++)
    {
        auto port = larg->associatedPorts.at(pidx);
        ether_addr tmp;
        ...
        std::string macString(macStr);
        bool found = false;
        rte_eth_macaddr_get(port, (ether_addr *)larg->srcs.at(pidx).mac);
        //since nb_ports < suppliedIp.size, assign port-th to suppliedIps
        IPFromString(srcIp, larg->srcs.at(pidx).ip);
